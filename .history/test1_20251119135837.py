...
async def place_market_close(client, symbol):
    try:
        ...
        if qty == 0:
            return
        data['closing'] = True
        side = direction_from_qty(qty)
        close_side = opposite(side)
...
async def place_stop_loss(client, symbol, side):
    try:
        data = get_positions_data()[symbol]
        ...
        price = quantize_tick(price, SYMBOL_FILTERS[symbol]["price_tick"])
        quantity = quantize_tick(quantity, SYMBOL_FILTERS[symbol]["qty_step"])
...
async def handle_new_position(client, symbol, qty, price, side):
    ...
    stop_loss_price = quantize_tick(stop_loss_price, SYMBOL_FILTERS[symbol]["price_tick"])
...
async def update_existing_position(symbol, order_qty, order_side, client=None):
    ...
    # Проверка на изменение позиции, если она не закрыта
    if old_size != updated_size and updated_size != 0 and client is not None:
        ...
        stop_side = opposite(direction_from_qty(updated_size))
        await place_stop_loss(client, symbol, stop_side)
